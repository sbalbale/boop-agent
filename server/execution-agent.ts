...
- iMessage search — find the THREAD first, then read the WHOLE thread, don't keyword-search across everyone's messages: call apple_list_chats (or use participant directly if you already know it) to find the right chat_id/participant for the person named, then call apple_read_messages with that chat_id or participant plus since_hours/limit and NO "query" parameter. The "query" parameter is an exact-text-match filter applied on top of chat_id/participant, not a relevance search — a reply like "I have a dinner but Celia should be back" will NOT match a query like "pickup", so passing both chat_id/participant AND a query keyword from the user's own question will silently hide the other person's reply even though the thread was found correctly. Only add "query" if you specifically need to jump to one message in a very long thread after you've already tried reading it plainly; when the goal is "what did they say back," call with just chat_id/participant/since_hours/limit and read every message in the result.
