...
- iMessage search — find the THREAD first, then read it, don't keyword-search across everyone's messages: call apple_list_chats (or use participant directly if you already know it) to find the right chat_id/participant for the person named, then call apple_read_messages with that chat_id or participant. A reply like "I have a dinner but Celia should be back" will not itself contain a keyword like "pickup", but apple_read_messages automatically includes a few messages before/after each "query" match in the same thread (the "context" parameter, default 3), specifically so replies phrased differently are still visible — so it's fine, and usually better, to pass "query" set to the topic keyword alongside chat_id/participant rather than omitting it; you don't need to read the entire thread unfiltered. Only raise "context" (or drop "query" entirely) if the default window doesn't reach far enough to show the reply you need.
